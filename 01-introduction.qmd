...
Vamos a usar R como lenguaje de programación y RStudio como una IDE (_Integrated Development Environment_), un Entorno Integrado de Desarrollo. Si no los tenés instalado en tu computadora, no te preocupes, tenemos este  [proyecto en RStudio Cloud](https://posit.cloud/content/6431286) para que puedas trabajar.

Vamos a trabajar con archivos de Quarto para generar nuestro reportes y poder guardar el código que genera nuestro análisis, así que vamos a ver qué es un documento Quarto. Te preparamos [este informe de ejemplo](https://github.com/yabellini/AgroReportesConR/blob/main/reporte_cultivares.qmd), por favor abrilo en RStudio o en RStudio Cloud. 


## Quarto

Un archivo Quarto es un archivo de texto plano, con algunas reglas y una sintaxis especial que nos permite escribir código y texto juntos. Cuando se previsualiza, el código se evaluará y ejecutará y el texto se formateará de manera que se cree un informe o documento reproducible que sea agradable de leer y que contenga todo tu trabajo.
...
* Debajo hay secciones alternas _blancas_ y _grises_. Estas son las dos secciones principales que componen un archivo Quarto:
      * Las secciones grises son el código R
      * Las secciones blancas son el texto de Markdown
* Hay texto negro, azul y verde.

> Sigamos adelante y "previsualicemos" el documento haciendo clic en laflechita azul (![](img/render-boton.png)) en la parte superior del archivo Quarto. 

¡Acabamos de crear un archivo html! Se trata de una única página web que estamos viendo localmente en nuestros propias computadoras. Al generar este documento Quarto, R ha formateado el texto markdown y ha ejecutado el código R.

![](img/markdown-knit.png){fig-alt="Quarto a la izquierda. Documento generado a la derecha."}
...
Esta tarea es un poco engorrosa si hay que cambiar muchas cosas cada vez que queremos volver a ejecutar el análisis para diferentes cultivares Pero no te preocupes, aprenderemos a hacer todo más automático al final del taller. 
